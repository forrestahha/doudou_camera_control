...
class EzvizControlTests(unittest.TestCase):
    ...
    def test_env_config_reads_capture_wall_timeouts(self):
        with tempfile.TemporaryDirectory() as tmp:
            env_path = Path(tmp) / "timeouts.env"
            env_path.write_text(
                "\n".join(
                    [
                        "EZVIZ_ACCESS_TOKEN='token'",
                        "EZVIZ_DEVICE_SERIAL='serial'",
                        "CB60_CAPTURE_WALL_TIMEOUT_SECONDS='120'",
                        "CB60_CAPTURE_WITH_LAS_WALL_TIMEOUT_SECONDS='900'",
                    ]
                ),
                encoding="utf-8",
            )
        ...
        self.assertEqual(config.capture_wall_timeout_seconds, 120.0)
