...
- `ffmpeg` 是录制和转码必需项
- `tesseract-ocr` 只用于异常帧 OCR 分析和问题说明
- **是否 accepted 不依赖 OCR**，没有 `tesseract` 不会直接让本来合格的视频变成异常
...
推荐在 OpenClaw 运行环境里额外确认：

```bash
python3 scripts/ezviz_cb60_control.py doctor
```

确认输出里的 `runtime_dependencies` 至少包含：

- `ffmpeg_installed=true`
- `tesseract_installed=true`
- `tos_sdk_installed=true`（如果要走云上 TOS/LAS）
...
- 插件仍然可以正常录制、转码、验片
- 只是异常帧分析里不会附带 OCR 文本
- 这不是 accepted / abnormal 的主判定条件

### 当前验片规则

当前插件的 accepted 规则已经固化为：

- 视频时长达到 `10` 秒及以上
- 分辨率达到竖屏主素材可用标准（宽高均至少 `1000`）

只要满足上面两条，就会判定为 `accepted`。
OCR 只用于异常时补充文字分析说明，不是 accepted 的前置条件。
...
对当前这台 `CB60字节测试（BG4238078）`，已经验证成功的拍摄链路是：
