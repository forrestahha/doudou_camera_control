...
19. When no direct `--stream-url` and no managed stream are provided, the workflow should default to `protocol=4`, `quality=1`, `supportH265=0`, `source=1`, and `type=1` so it prefers the H264-compatible FLV address shape that is least likely to return the EZVIZ H265 placeholder frame.
